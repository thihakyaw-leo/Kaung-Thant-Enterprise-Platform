...
inventoryAPI.post('/categories', async (c) => {
  const db = drizzle(c.env.DB);
  const body = await c.req.json();
  const now = Math.floor(Date.now() / 1000);
  ...
  try {
    await db.insert(schema.inventoryCategory).values({
      id: categoryId,
      name: body.name,
...
inventoryAPI.put('/categories/:id', async (c) => {
  const db = drizzle(c.env.DB);
  ...
  try {
    await db.update(schema.inventoryCategory)
      .set({
        name: body.name,
        parentId: body.parentId || null,
        updatedAt: now,
      ...
      .where(eq(schema.inventoryCategory.id, id));
...
inventoryAPI.delete('/categories/:id', async (c) => {
  const db = drizzle(c.env.DB);
  const id = c.req.param('id');

  try {
    await db.delete(schema.inventoryCategory).where(eq(schema.inventoryCategory.id, id));
...
inventoryAPI.post('/units', async (c) => {
  const db = drizzle(c.env.DB);
  const body = await c.req.json();
  const now = Math.floor(Date.now() / 1000);
  ...
  try {
    await db.insert(schema.inventoryUnit).values({
      id: unitId,
      name: body.name,
    ...
    return c.json({ success: true, id: unitId });
  } catch (error: unknown) {
    const message = error instanceof Error ? error.message : 'Unknown error';
    return c.json({ success: false, error: message }, 500);
  }
...
inventoryAPI.put('/units/:id', async (c) => {
  const db = drizzle(c.env.DB);
  const id = c.req.param('id');
  const body = await c.req.json();
  const now = Math.floor(Date.now() / 1000);
  ...
  try {
    await db.update(schema.inventoryUnit)
      .set({
        name: body.name,
        shortName: body.shortName,
        updatedAt: now,
      ...
      .where(eq(schema.inventoryUnit.id, id));
    return c.json({ success: true, id });
  } catch (error: unknown) {
    const message = error instanceof Error ? error.message : 'Unknown error';
    return c.json({ success: false, error: message }, 500);
...
inventoryAPI.delete('/units/:id', async (c) => {
  const db = drizzle(c.env.DB);
  const id = c.req.param('id');

  try {
    await db.delete(schema.inventoryUnit).where(eq(schema.inventoryUnit.id, id));
    return c.json({ success: true });
  } catch (error: unknown) {
    const message = error instanceof Error ? error.message : 'Unknown error';
    return c.json({ success: false, error: message }, 500);
