...
// Apply RBAC: Only saas admins/super_admins can access master API routes
masterAPI.use('*', async (c, next) => {
  const user = c.get('user');
  if (!user || !['admin', 'super_admin', 'owner'].includes(user.role)) {
    return c.json({ success: false, error: 'Forbidden: SaaS admin access required' }, 403);
  }
  await next();
});
...
/**
 * --------------------------------------------------------------------------------
 * PROFILE MANAGEMENT (Current Admin)
 * --------------------------------------------------------------------------------
 */
...
masterAPI.patch('/me', async (c) => {
  ...
  const filtered = Object.keys(updates)
    ...
    .reduce((obj: any, key) => { obj[key] = updates[key]; return obj; }, {});

  try {
    await db.update(schema.saasUsers)
      .set(filtered)
      .where(eq(schema.saasUsers.id, user.id));
    return c.json({ success: true });
  } catch (error: unknown) {
    const message = error instanceof Error ? error.message : 'Unknown error';
    return c.json({ success: false, error: message }, 500);
...
masterAPI.patch('/me/password', async (c) => {
  const db = drizzle(c.env.DB);
  const user = c.get('user');
...
masterAPI.get('/users', async (c) => {
  const db = drizzle(c.env.DB);
  try {
    const users = await db.select().from(schema.saasUsers);
    return c.json({ success: true, data: users.map(({ passwordHash, ...u }) => u) });
  } catch (error: unknown) {
    const message = error instanceof Error ? error.message : 'Unknown error';
    return c.json({ success: false, error: message }, 500);
  }
...
masterAPI.patch('/users/:id', async (c) => {
  ...
  // Whitelist: prevent mass-assignment of passwordHash or id
  const ALLOWED_USER_FIELDS = ['fullName', 'email', 'role', 'status', 'permissions', 'avatarUrl'];
  const updates = Object.fromEntries(
    Object.entries(body).filter(([k]) => ALLOWED_USER_FIELDS.includes(k))
  );
  ...
  try {
    for (const { key, value } of updates) {
      await db.insert(schema.systemSettings)
        .values({ key, value, category: 'general', updatedAt: now })
        .onConflictDoUpdate({
...
masterAPI.get('/health', async (c) => {
  ...
  try {
    ...
    try {
      if (c.env.KV_CACHE) await c.env.KV_CACHE.get('health_check');
      else kvStatus = 'outage';
    } catch (e) {
      kvStatus = 'degraded';
    }
    ...
    // 4. API Request Latency
    const apiLatency = Date.now() - startTime;

    // 5. Cloudflare Edge Location Data
    const colo = (c.req.raw.cf?.colo as string) || 'Edge';
    const city = (c.req.raw.cf?.city as string) || 'Global';

    // 6. Real Analytics from Cloudflare Analytics Engine
    let requestsPerSecond = 0;
    let errorRate = '0%';

    if (c.env.CLOUDFLARE_ACCOUNT_ID && c.env.CLOUDFLARE_API_TOKEN) {
      try {
        const query = `SELECT SUM(_sample_interval) as total_reqs, SUM(IF(blob3 != '200' AND blob3 != '201' AND blob3 != '204', _sample_interval, 0)) as errors FROM kaungthant_api_metrics WHERE timestamp >= NOW() - INTERVAL '1' MINUTE`;
        const res = await fetch(`https://api.cloudflare.com/client/v4/accounts/${c.env.CLOUDFLARE_ACCOUNT_ID}/analytics_engine/sql`, {
          method: 'POST',
        ...
        if (res.ok) {
          ...
          if (json.data && json.data.length > 0) {
            const total = Number(json.data[0].total_reqs) || 0;
            const errors = Number(json.data[0].errors) || 0;
            requestsPerSecond = Math.round(total / 60);
            errorRate = total > 0 ? ((errors / total) * 100).toFixed(2) + '%' : '0%';
          }
        }
      } catch (e) {
        // Silently ignore if analytics fail so health check doesn't crash
      }
    }
    ...
    return c.json({ 
      ...
      data: {
        status: 'operational',
        services: {
          api: { status: 'operational', latency: `${apiLatency}ms`, uptime: '100%' },
          database: { status: 'operational', queryTime: `${d1Latency}ms`, connections: 0 },
          storage: { status: storageStatus, throughput: 'Serverless', availability: '99.9%' },
          cache: { status: kvStatus, hitRate: 'N/A', size: 'Serverless' }
        },
        metrics: {
          cpu: 0,
          memory: 0,
          requestsPerSecond,
          errorRate,
          totalTenants: tenantStats?.[0]?.count || 0,
...
masterAPI.get('/assets/:key{.+}', async (c) => {
  const key = c.req.param('key');
  try {
    const object = await c.env.R2_ARCHIVE.get(key);
